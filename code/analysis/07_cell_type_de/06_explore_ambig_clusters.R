...
pdf(here("plots", "07_cell_type_de", "umap_cell_types.pdf"))
plotReducedDim(spe, dimred="UMAP", colour_by="cell_types")
plotReducedDim(spe, dimred="UMAP", colour_by="cell_area")
plotReducedDim(spe, dimred="UMAP", colour_by="BrNum")
plotReducedDim(spe, dimred="UMAP", colour_by="negProbe_percent")
dev.off()
...
spe_pseudo <- registration_pseudobulk(
    ...
    ) # need to rerun 01_create_cell_type_pseudobulk
...
ComplexHeatmap::Heatmap(plot_counts, cluster_columns=dend,
        name="logcounts", bottom_annotation = column_ha,
